...
df3 =pd.read_excel('C:/Users/jakub/OneDrive/Documents/NeuralNetworks/Data/Predictions.xlsx', na_values=["NA"], usecols = "A:J")
...
def destandardise(column_name):
    ...
    if(column_name=="Prediction"):
        max = df2.loc[0, "Index flood"]
        min = df2.loc[1, "Index flood"]
    else:
        max = df2.loc[0, column_name]
        min = df2.loc[1, column_name]

    # compute de-standardised value at every row
    for i, standard_value in df3[column_name].iteritems():
        original_value = ((standard_value-0.1)/0.8)*(max-min)+min
        df3.loc[i, column_name] = original_value

# iterate over columns and apply function
for column_name in df:
    destandardise(column_name)
destandardise("Prediction")

# write back to excel
df3.to_excel('C:/Users/jakub/OneDrive/Documents/NeuralNetworks/Data/Predictions.xlsx', index=False)
